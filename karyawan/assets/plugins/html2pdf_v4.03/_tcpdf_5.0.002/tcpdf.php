	...
	*/
	class TCPDF {
		...
		public function unichr($c) {
			// Pastikan nilai yang diberikan adalah integer
			if (!is_int($c)) {
				// Ubah argumen menjadi integer jika memungkinkan
				$c = intval($c);
			}
		
			// Kemudian, lanjutkan dengan pengecekan seperti biasa
			if (!$this->isunicode) {
				return chr($c);
			} elseif ($c <= 0x7F) {
				// one byte
		...
		public function get_mqr() {
			...
			if (PHP_VERSION_ID < 50300) {
				return @set_magic_quotes_runtime($mqr);
...
if ($fileContent !== false) {
    ...
    if ($img !== false) {
        $imgalpha = imagecreatetruecolor($wpx, $hpx);
    } else {
        echo "Gagal membuat gambar dari file: $file";
    }
} else {
    echo "Gagal membaca file: $file";
}
		...
		protected function UTF8StringToArray($str) {
			...
			$length = strlen($str);
			for ($i = 0; $i < $length; ++$i) {
				$char = ord($str[$i]); // get one string character at a time
